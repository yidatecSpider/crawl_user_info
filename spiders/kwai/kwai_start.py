...
class KwaiStartTopSpider(object):
    ...
    def extract_user_info(self) -> None:
        ...
        解析用户信息
        """
        user_list = [rank_item.get('eid') for rank_item in self.rank_list]
    ...
    def save_rank_data(self) -> None:
        ...
        sql = None
        try:
            sql = '''insert into kwai_star_day(`crawl_date`,`video_name`,`rank_update_time`,`rank_num`,`rank_name`,`total_subscribe_count`,
            `user_id`,`eid`) VALUES (substr(Now(),1,10),%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE `video_name` = VALUES(
            `video_name`),`user_id` = VALUES(`user_id`),`total_subscribe_count` = VALUES(`total_subscribe_count`),
            `rank_update_time` = VALUES(`rank_update_time`),`eid` = VALUES(`eid`) '''
