...
from spiders.kwai.user_id import get_user_page_url
...
class BaseUserInfoExtractor(object):
    ...
    def __init__(self):
        self.api = "http://wxmini-api.uyouqu.com/rest/wd/wechatApp/user/profile?__NS_sig3=a2b2f6c592a9af9391fffcfd6a35adff99bdbb87e3e3e1e1eeefecf6&__NS_sig3_origin=3sCt3iAAAAAAAAAAAAAAAwEQBv2b8ewCcXuIaSAAAAAL0U0qjuzPjugDr617wa/H1dwkx/+rVV0Mh5kTn+qa8Q=="
        self.user_info_list = []
    ...
    def extract_user_info(self, rsp, eid):
        user_info = {}
        ...
        user_info['url'] = f"https://www.kuaishou.com/profile/{eid}"
    ...
    def save_user_info(self):
        ...
        try:
            ...
            ,`constellation`,`identity`,`nickname`,`sex`,`intro`,`url`,`crawl_source`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) 
            ...
            `fans_count` = VALUES(`fans_count`),`like_count` = VALUES(`like_count`),`area` = VALUES(`area`)
             ,`identity` = VALUES(`identity`),`nickname` = VALUES(`nickname`),`intro` = VALUES(`intro`),`url` = VALUES(`url`)'''
    ...
    def process(self, user_id_list) -> None:
        ...
        """
        [self.extract_user_info(self.download_user_info(user_id), user_id) for user_id in user_id_list]
        self.save_user_info()
